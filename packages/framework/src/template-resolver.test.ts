...
const FakeTemplate: TemplateComponent = ({ head: _head }: { head: ReactNode }) => null;
...
test("throws with available template list when ID is not found", () => {
	const MarketingTemplate: TemplateComponent = ({ head: _head }: { head: ReactNode }) => null;
	...
	expect(() => resolveTemplate("foo", registry)).toThrow(
